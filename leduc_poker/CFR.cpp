...
double CFR::mccfr(const int targetPlayer, const unsigned int iteration, const std::vector<int>& holeCards, const std::vector<int>& streetCards, const int lastBet, int pot, int passedStreets, int sinceChance, int infoset, int numPastActions) {
    const int curPlayer = (numPastActions % 2 == 0) ? 0 : 1;

    double nodeUtil = 0.0; // MOVE DOWN
    ...
    if (curPlayer != targetPlayer) {
        ...
        else if (action == 0b010) bet = lastBet;
        pot += bet;

        return -mccfr(targetPlayer, iteration, holeCards, streetCards, bet, pot, passedStreets, sinceChance, infoset ^ action, numPastActions + 1);
    }
    ...
    for (int i = 0; i < numActions; ++i) {
        ...
        cfUtils[i] = -mccfr(targetPlayer, iteration, holeCards, streetCards, bet, pot, passedStreets, sinceChance, infoset ^ action, numPastActions + 1);
        pot -= bet;
        if (cfUtils[i] == -pot / 2) cfUtils[i] *= -1;
...
int CFR::terminal_util(const int curPlayer, const int pot, const std::vector<int>& holeCards, const std::vector<int>& streetCards, int passedStreets) const {
    int playerCard = holeCards[curPlayer], oppCard = holeCards[!curPlayer];
    if (passedStreets) {
        for (int streetCard : streetCards) {
            if (playerCard == streetCard) return pot;
            else if (oppCard == streetCard) return -pot;
        }
    }
    return playerCard == oppCard ? pot / 2 : playerCard > oppCard ? pot : -pot;
}
